...
Write the code approved in `/cmd-plan`. No scope expansion — implement exactly what was planned.

Requires `/cmd-plan` to have been approved.

## Rules during implementation

- **One file at a time.** Complete each file fully before moving to the next.
- **No scope creep.** If you notice something unrelated that could be improved, note it in `/cmd-review` — do not fix it now.
